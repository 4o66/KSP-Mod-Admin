...
/// <summary>
/// The progress changed callback funktion.
/// </summary>
...
/// <summary>
/// The AsyncTask class wraps the System.ComponentModel.BackgroundWorker class to perform asyncrone tasks (Sample on bottom of src).
/// </summary>
/// <typeparam name="T_RETURN_VALUE">Type of the return value of the run function.</typeparam>
/// <remarks></remarks>
public class AsyncTask<T_RETURN_VALUE>
{
    ...
    /// <summary>
    ///  The Exception that occured during execution of the task function.
    /// </summary>
    ...
    /// <summary>
    /// Set this property within your task function it will call the progress changed callback funktion with the passed value.
    /// </summary>
    ...
    /// <summary>
    /// This function calls the passed task function asyncronously.
    /// </summary>
    /// <param name="task">The task function to execute asyncron. (A function with the signiture "T_RETURN_VALUE FunctionName&lt;T_RETURN_VALUE&gt;()")</param>
    /// <param name="finished">The finished callback funktion. (A function with the signiture "void FunctionName&lt;T_RETURN_VALUE&gt;(T_RETURN_VALUE result, Exception ex)")</param>
    /// <param name="progressChanged">The progress change callback funktion. (A function with the signiture "void FunctionName(int percentage)")</param>
    ...
    /// <summary>
    /// Starts a asyncron download.
    /// </summary>
    /// <param name="url">Download link</param>
    /// <param name="downloadPath">Path and filename to download to.</param>
    /// <param name="finished">The finished callback funktion. (A function with the signiture "void FunctionName&lt;bool&gt;(bool result, Exception ex)")</param>
    /// <param name="progressChanged">The progress change callback funktion. (A function with the signiture "void FunctionName(int percentage)")</param>
    ...
    /// <param name="supportCancellation"></param>
    public void SetCallbackFunctions(AsyncHandler<T_RETURN_VALUE> task, AsyncResultHandler<T_RETURN_VALUE> finished, AsyncProgressChangedHandler progressChanged, bool supportCancellation = false)
    {
    ...
    /// <param name="finished">The finished callback funktion. (A function with the signiture "void FunctionName&lt;bool&gt;(bool result, Exception ex)")</param>
    /// <param name="progressChanged">The progress change callback funktion. (A function with the signiture "void FunctionName(int percentage)")</param>
    public void SetDownloadCallbackFunctions(string url, string downloadPath, AsyncResultHandler<bool> finished, AsyncProgressChangedHandler progressChanged)
    {
        m_URL = url;
    ...
    /// <summary>
    /// Runs the task function asyncron (with a BackgroundWorker).
    /// </summary>
    public void Run()
    {
        if (m_Worker != null)
    ...
    /// <summary>
    /// Runs the download asyncron (with a WebClient).
    /// </summary>
    ...
    /// <summary>
    /// Cancels the execution of the task funktion if possible.
    /// </summary>
    /// <returns>True if cancellation was successfull otherwise false.</returns>
    public bool Cancel()
    {
        bool result = false;
    ...
    private void DownloadFinished(object sender, AsyncCompletedEventArgs e)
    {
        ...
        {
            if (e.Cancelled)
                m_DownloadFinished(false, new Exception("Download cancelled."));
            else
 ...
 *                     {
 *                          // do your work here
 *                          // i will compare appels with pears =)
 *                          int appel = 0;
 *                          int pear = 1;
 *                          return (appel == pear);
 *                     },
