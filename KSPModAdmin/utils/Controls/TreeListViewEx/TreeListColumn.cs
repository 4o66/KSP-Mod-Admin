...
namespace KSPModAdmin.Utils.CommonTools
{
	...
	public class TreeListColumn
	{
        ...
        [DefaultValue(false)]
        public bool ReadOnly = false; // Added by BH

		public TreeListColumn(string fieldName, bool isReadOnly = false)
		{
			m_fieldName = fieldName;
            ReadOnly = isReadOnly; // Added by BH
		}
        public TreeListColumn(string fieldName, string caption, bool isReadOnly = false)
		{
			m_fieldName = fieldName;
            m_caption = caption;
            ReadOnly = isReadOnly; // Added by BH
		}
        public TreeListColumn(string fieldName, string caption, int width, bool isReadOnly = false)
		{
			m_fieldName = fieldName;
			m_caption = caption;
            m_width = width;
        ...
        public TreeListColumn(string fieldName, string caption, int width, int minWidth, bool fixedWidth = false, bool isReadOnly = false)
        {
            m_fieldName = fieldName;
            m_caption = caption;
            m_width = width;
        ...
        public int mMinWidth = 20;
        public int MinWidth { get { return mMinWidth; } set { mMinWidth = value; } }

        public bool mFixedWidth = false;
        public bool FixedWidth { get { return mFixedWidth; } set { mFixedWidth = value; } }

        public bool mSortDesc = false;
        public bool SortDesc { get { return mSortDesc; } set { mSortDesc = value; } }
	...
	public class TreeListColumnCollection : IList<TreeListColumn>, IList
	{
		...
		public TreeListColumnCollection(TreeListViewEx owner)
		{
			m_owner = owner;
        ...
        public void UpdateColumnSizes()
        {
            ...
            int controlWidth = m_owner.ClientRectangle.Width - 2;
            //int[] ColWidths = new int[this.Count];
            //int[] ColMinWidths = new int[this.Count];
            int allColumnsWidth = 0;
            int nonFixedWidthColumnCount = 0;
            for (int i = 0; i < this.Count; ++i)
            {
                var col = this[i];
                //ColWidths[i] = col.Width;
                //ColMinWidths[i] = col.MinWidth;
                allColumnsWidth += col.MinWidth;
                if (!col.FixedWidth)
                    ++nonFixedWidthColumnCount;
            }

            int diff = controlWidth - allColumnsWidth;
            if (allColumnsWidth == controlWidth || nonFixedWidthColumnCount == 0)
                return;

            else
            {
                var i = diff / nonFixedWidthColumnCount;
                foreach (TreeListColumn col in this)
                {
                    if (!col.FixedWidth)
                    {
                        if (col.MinWidth + i >= col.MinWidth)
                            col.Width = col.MinWidth + i;
                        else
