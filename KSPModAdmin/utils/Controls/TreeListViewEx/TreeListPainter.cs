...
namespace KSPModAdmin.Utils.CommonTools
{
	...
	{
		protected TreeListViewEx m_owner;
		public CellPainter(TreeListViewEx owner)
		{
			m_owner = owner;
		}
        ...
        // Changed by BH
		public virtual void PaintCell(Graphics dc, Rectangle cellRect, Node node, TreeListColumn column, TreeList.TextFormatting format, object data)
		{
            PaintCellBackground(dc, cellRect, column, format);
        ...
        // Changed by BH
        public virtual void PaintCellBackground(Graphics dc, Rectangle cellRect, TreeListColumn column, TreeList.TextFormatting format)
        {
		...
		public virtual void PaintCellPlusMinus(Graphics dc, Rectangle glyphRect, Node node)
		{
            if (!Application.RenderWithVisualStyles)
            {
                ...
                glyphRect.Y -= 1;
                if (node.Expanded)
                    dc.DrawImageUnscaled(Properties.Resources.minus, glyphRect);
                else
                    dc.DrawImageUnscaled(Properties.Resources.plus, glyphRect);
            }
            else
            {
